...
| `num_speakers` | число | — | точное число участников, если известно |
| `min_speakers` / `max_speakers` | число | — | либо диапазон вместо точного числа |
| `exclusive` | bool | `true` | брать `exclusive_speaker_diarization`: один говорящий в момент времени |
...
## Выравнивание слов по спикерам

pyannote отдаёт интервалы «кто когда говорил», ASR — слова с тайм-кодами; дальше
каждое слово надо приписать спикеру. Тонкие места:

- **Подсказка числа участников.** `num_speakers` (или `min_speakers`/`max_speakers`)
  передаётся в pipeline. Помогает, когда автоопределение ошибается; если оно и так
  право, разницы не будет. Для звонков «агент + клиент» имеет смысл всегда ставить 2.
- **Exclusive-разметка.** По умолчанию берётся `exclusive_speaker_diarization`: в
  каждый момент ровно один говорящий, слово на стыке не приходится делить. На
  записи совещания это убрало 2 разорванные реплики из 7 — эффект тем заметнее,
  чем больше в записи перебиваний.
- **Вырожденные тайм-коды.** ASR иногда отдаёт слову одинаковые start и end. С
  нулевой длиной перекрытие с любым turn'ом равно нулю, и слово уезжает к соседу.
  Такие интервалы расширяются до 0.3 с вокруг середины (`MIN_WORD_S`).
- **Спорные слова.** Если два turn'а перекрывают слово почти поровну (разница
  меньше `TIE_S` = 0.05 с), слово наследует спикера **следующего** уверенно
  определённого слова: обрывок на стыке принадлежит начинающейся реплике.

Замер на 42-минутном совещании (3 участника): сегментов 233 → 224, сегментов
нулевой длины 7 → 3, рваных стыков 14 → 13, распределение речи по спикерам не
изменилось.
