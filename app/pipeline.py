...
LOW_CONF = float(os.getenv("LOW_CONF", "0.7"))  # ниже — слово помечаем как ненадёжное
# 0,7, а не 0,6: фамилии и названия компаний модель отдаёт с уверенностью
# 0,60–0,70 («Кросс» 0,61, «Живоглядова» 0,65, «Ромашка» 0,65) — при пороге 0,6
# они оставались белыми, хотя как раз их и надо проверять глазами.
